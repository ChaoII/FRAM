...
class FaceRecognizeThread(QThread):
    ...
    rec_result_signal = pyqtSignal(int)
        ...
        while not self._is_stop:
            if self._cap.isOpened():
                ...
                if self._is_single and face_nums > 1:
                    print("当前人数" + "face_nums" + ",目前仅支持单人脸解锁。。。。。。")
                labels = []
                if self._record_nums == self._record_freq:
                ...
                for i, data in enumerate(faces_result):
                    ...
                    if face_id != self._last_id or self._frame_nums == self._ignore_nums:
                        ...
                        # -------------信号发送逻辑请在这里编写------------------
                        signal = 1 if ret.confidence > self._threshold else 0
                        # 发送io信号-----该处作为打卡可能不需要了
                        self.rec_result_signal.emit(signal)
